...
extends Module {
    ...
    public void drawTargetHud(float partialTicks, EntityPlayer player) {
        ...
        EntityPlayer entityPlayer = AutoCrystal.target != null ? AutoCrystal.target : (target = Killaura.target instanceof EntityPlayer ? (EntityPlayer)Killaura.target : Target.getClosestEnemy());
        if (target == null) {
            return;
        }
        if (target.isDead) {
