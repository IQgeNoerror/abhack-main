...
extends Module {
    ...
    private final Setting<Mode> mode = this.register(new Setting<Mode>("Mode", Mode.FLAT, v -> this.settings.getValue() == Settings.PATTERN));
    ...
    private final Setting<Float> range = this.register(new Setting<Float>("Range", Float.valueOf(5.0f), Float.valueOf(1.0f), Float.valueOf(6.0f), v -> this.settings.getValue() == Settings.PLACE));
    private final Setting<Integer> blocksPerTick = this.register(new Setting<Integer>("Blocks/Tick", Integer.valueOf(8), Integer.valueOf(1), Integer.valueOf(8), v -> this.settings.getValue() == Settings.PLACE));
    private final Setting<Integer> placeDelay = this.register(new Setting<Integer>("PlaceDelay", Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(500), v -> this.settings.getValue() == Settings.PLACE));
    private final Setting<Boolean> rotate = this.register(new Setting<Boolean>("Rotate", Boolean.valueOf(false), v -> this.settings.getValue() == Settings.PLACE));
    ...
    private final Setting<Boolean> packet = this.register(new Setting<Boolean>("Packet", Boolean.valueOf(false), v -> this.settings.getValue() == Settings.PLACE));
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        this.placepositions.forEach(pos -> RenderUtil.drawSexyBoxOyVeyIsRetardedFuckYouESP(new AxisAlignedBB(pos), box, outline, this.lineWidth.getValue().floatValue(), this.outline.getValue(), this.box.getValue(), 1.0f, 1.0f, 1.0f));
    }
