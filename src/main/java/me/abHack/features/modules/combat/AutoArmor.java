...
extends Module {
    ...
    private final Setting<Boolean> mendingTakeOff = this.register(new Setting<Boolean>("AutoMend", true));
    ...
    private final Setting<Integer> repair = this.register(new Setting<Object>("Repair%", Integer.valueOf(90), Integer.valueOf(1), Integer.valueOf(100), v -> this.mendingTakeOff.getValue()));
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            ItemStack chest;
            int slot4;
            if (this.mendingTakeOff.getValue().booleanValue() && InventoryUtil.getItemHotbar(Items.EXPERIENCE_BOTTLE) != -1 && AutoXP.INSTANCE.isEnabled() && AutoXP.INSTANCE.bind.getValue().isDown() && AutoArmor.mc.world.playerEntities.stream().noneMatch(e -> e != AutoArmor.mc.player && !OyVey.friendManager.isFriend(e.getName()) && AutoArmor.mc.player.getDistance((Entity)e) <= (float)this.closestEnemy.getValue().intValue()) && !this.flag) {
                int goods;
                int dam;
                int takeOff = 0;
