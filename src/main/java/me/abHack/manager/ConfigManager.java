...
public class ConfigManager implements Util {
    ...
    public String config = "ab-Hack/config/";
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("ab-Hack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("ab-Hack/" + name + "/"))) {
            this.config = "ab-Hack/" + name + "/";
        } else {
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("ab-Hack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("ab-Hack/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("ab-Hack/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("ab-Hack", ""));
                writer.close();
            } else {
